...
# the file sizes became too big for jupyter. This code needs to be runned three times
# to clear the RAM. The following input decides what image should be downloaded.
...
def importdata(aoi, daterange):
    ...
    #########################
    # for every future itemset, create a temporary item and add this to the allready
    # existing item set.
...
# extract 2009 and 1999 from the itemcollection and create three individual ones
traincheck,valcheck = 0, 0
r = []
for i, c in zip(charts, range(len(charts))):
    if i.datetime.year == 2009:
        #print(i, c)
        if traincheck == 0: # first one
...
#----------------------------------------------------------
# compute and make deployment file
create_file(ds = ds, format = format)
print('charts done')
# compute and make train file
create_file(ds = dt, format= format, datause='train_data_')
print('train done')
# compute and make validation file
